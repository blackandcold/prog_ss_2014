...
/**
 * Takes two integers from the data stack and applies logical AND to them.
 * If the integers are not binary (0 or 1) an error is reported.
 */
public class LogicalAndOperator implements IElement {
	...
	public void apply(Context ctxt) {
		...
		if (i1 < 0 || i1 > 1 || i2 < 0 || i2 > 1)
			throw new CalculatorException("Arguments for logical and have to be binary!");
